...
export const generatePDFFoods = (foods) => {
  const doc = new jsPDF();
  ...
  doc.setFontSize(14);
  doc.setFont("times", "bold");
  doc.text("Laporan Data Makanan", 105, 45, { align: "center" });

  // Table Data
  autoTable(doc, {
    startY: 55, // Posisi mulai tabel setelah judul
    head: [["Food ID", "Name", "Price"]],
    body: foods.map((food) => [
      food.id,
      food.name,
      `Rp ${food.price.toLocaleString("id-ID")}`, // Format harga
    ]),
    styles: { font: "times", fontSize: 10, cellPadding: 3 },
    headStyles: { fillColor: [0, 122, 255], textColor: [255, 255, 255] }, // Header table biru putih
    alternateRowStyles: { fillColor: [230, 245, 255] }, // Warna selang-seling biru muda
  ...
  // Save PDF
  doc.save("laporan_data_makanan.pdf");
};

export const generatePDFUsers = (users) => {
  const doc = new jsPDF();
  ...
  doc.setFontSize(16);
  doc.setFont("times", "bold");
  doc.text("Catering", 105, 12, { align: "center" });

  doc.setFontSize(10);
  doc.setFont("times", "normal");
  doc.text(
    "Jl. Gedong Gg, RT 03 RW 02, Kec. Kramat Jati, Kota Jakarta Timur",
    105,
    18,
    { align: "center" }
  );
  ...
  doc.setFontSize(14);
  doc.setFont("times", "bold");
  doc.text("Laporan Data Pengguna", 105, 45, { align: "center" });

  // Table Data
  autoTable(doc, {
    startY: 55, // Posisi mulai tabel setelah judul
    head: [["User ID", "Nama", "Email", "Role"]],
    body: users.map((user) => [user.id, user.username, user.email, user.role]),
    styles: { font: "times", fontSize: 10, cellPadding: 3 },
    headStyles: { fillColor: [0, 122, 255], textColor: [255, 255, 255] }, // Header table biru putih
    alternateRowStyles: { fillColor: [230, 245, 255] }, // Warna selang-seling biru muda
  });
  ...
  // Save PDF
  doc.save("laporan_data_pengguna.pdf");
};

export const generatePDFOrdersApprove = (orders) => {
  const doc = new jsPDF();
